<div class="border-2 rounded-md mt-4 flex justify-between flex-col relative shadow-gray-400 shadow-lg @if($authorized && !$bookmaker['active']) border-red-500 @else border-gray-400 @endif">
   ...
   <div class="justify-start flex md:hidden border-b-2 border-gray-400">
        <a href="{{$bookmaker['url']}}" target="_blank" class="flex">
    ...
    <div class="flex justify-between">
        ...
        <div class="flex flex-col justify-around self-center mr-2">
            <a href="{{$bookmaker['url']}}" target="_blank" class="h-10 w-28 bg-blue-700 text-xs sm:text-sm text-white rounded-full hover:bg-blue-600 flex justify-center items-center cursor-pointer transition-all duration-500  my-4 sm:my-2">@lang('index.Claim') &#8594;</a>
            <a href="{{$bookmaker['url']}}" class="text-gray-700 text-xs sm:text-sm underline text-center mb-4 sm:mb-auto">@lang('index.Visit') {{$bookmaker['name']}}</a>
        </div>
    </div>
</div>
